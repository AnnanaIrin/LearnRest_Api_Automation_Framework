...
public class Tweet2  extends RestBase {
    ...
    public final String SINGLE_USER_TWEET_ENDPOINT="users/:id/tweets";
    ...
    public ValidatableResponse getSingleUserTweet(long id){
        ...
        return given().contentType("application/json").auth().oauth(this.apiKey, this.apiSecretKey, this.accessToken, this.accessTokenSecret).when().get(this.baseUrl + "/users/:"+id+"/tweets").then();
    }
    ...
    public ValidatableResponse postUsersIdFollowed(Object listId){
        return given().auth().oauth(this.apiKey, this.apiSecretKey, this.accessToken, this.accessTokenSecret)
                ...
                .body(listId)
                .when().post(this.baseUrl+POST_USERS_ID_FOLLOWING).then();
    }
